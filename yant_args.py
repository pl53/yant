...
def parse(args):
    ...
    parser_create.add_argument("-t", "--tag", metavar="tag",
                               default="", help="A tag of the new book")

    parser_add = subparsers.add_parser("add", help="Add a new note, tag, or command to book")
    parser_add.add_argument("-b", "--book", metavar="book",
                            required=True, help="book name")
    group_add = parser_add.add_mutually_exclusive_group(required=True)
    group_add.add_argument("-n", "--note", metavar="note", \
                           help="note to be added, use @# as delimiter")
    group_add.add_argument("-t", "--tag", metavar="tag", help="tag to be added")
    group_add.add_argument("-c", "--cmd", metavar="command", help="command to be added")
    ...
    group_del.add_argument("-n", "--note", metavar="note_title", \
                           help="title of the note to be deleted")
    group_del.add_argument("-t", "--tag", metavar="tag", help="tag to be deleted")
    group_del.add_argument("-c", "--cmd", metavar="command", help="command to be deleted")
    ...
    group_find = parser_find.add_mutually_exclusive_group(required=True)
    ...
    group_review = parser_review.add_mutually_exclusive_group(required=True)
    ...
    parser_import.add_argument("-b", "--book", metavar="book",
                            required=True, help="book name")
    parser_import.add_argument("--file", metavar="file",
                            required=True, help="file to import from")

    parser_export = subparsers.add_parser("export", help="export notes to file")
    parser_export.add_argument("-b", "--book", metavar="book",
                            required=True, help="book name")
    parser_export.add_argument("--file", metavar="file",
                            default="-", help="exporting file, stdout by default")
